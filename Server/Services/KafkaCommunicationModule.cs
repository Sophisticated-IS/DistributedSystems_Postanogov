...
public sealed class KafkaCommunicationModule : CommunicationModule
{
    ...
    public override async Task<Message> GetMessageAsync(CancellationToken cancellationToken)
    {
        ...
        { 
            GroupId = "normalization-client-topic",
        ...
        consumer.Subscribe("normalization-client-topic");

        var consumeResult = await Task.Run(() => consumer.Consume(cancellationToken), cancellationToken).ConfigureAwait(false);
    ...
    public override async Task SendMessageAsync(Message message, CancellationToken cancellationToken)
    {
        ...
        await _producer.ProduceAsync("normalization-server-topic", new Message<Null, byte[]>
        {
            Value = bytesMsg
